...
def scan(
    ...
    if not report_url:
        try:
            ...
            try:
                analysis_result = analyze_bytes(file_bytes, filename=fname)

                redis_client = get_redis()
                file_cache_key = file_data_key(sha256)
                redis_client.setex(file_cache_key, 86400, json.dumps({
                    "filename": fname,
                    "mime_type": mime_type,
                    "size_bytes": len(file_bytes),
                    "sha256": sha256,
                ...
                print(f"Analysis failed for Gmail attachment {fname}: {e}")
                report_url = f"http://localhost:8000/r/gmail_{req.message_id}"
                
        except Exception as e:
            print(f"Failed to create share for Gmail attachment: {e}")
            report_url = f"http://localhost:8000/r/gmail_{req.message_id}"

    return {"ok": True, "report_url": report_url, "filename": fname}
